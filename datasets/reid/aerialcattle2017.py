...
class AerialCattle2017(DatasetBase):
    """
    Aerial Cattle 2017 Dataset
    
    Reference:
    Andrew, W., Hannuna, S., Campbell, N., & Burghardt, T. (2017).
    Automatic individual Holstein Friesian cattle identification via selective 
    local coat pattern matching in RGB-D imagery.
    In 2017 IEEE International Conference on Image Processing (ICIP).
    
    Dataset statistics:
        - images: 1379 (train) + 352 (query) + 598 (gallery)
        - identities: 15 (train) + 8 (query) + 8 (gallery)
    """
    def __init__(self, cfg, domain_label, verbose=True):
        self._dataset_dir = "AerialCattle2017"
        root = cfg.DATASET.ROOT
        self._dataset_path = os.path.join(root, self._dataset_dir)
        print("dataset path: ", self._dataset_path)
        self._domain = "aerial_cattle"
