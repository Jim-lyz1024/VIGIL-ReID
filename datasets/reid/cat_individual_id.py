...
class CatIndividualImages(DatasetBase):
    """
    CatIndividualImages
    ...
    Dataset statistics:
        - images: 7814 (train) + 1831 (query) + 3376 (gallery)
        - identities: 301 (train) + 208 (query) + 208 (gallery)
    """
    def __init__(self, cfg, domain_label, verbose=True):
        self._dataset_dir = "CatIndividualImages"
        root = cfg.DATASET.ROOT
        self._dataset_path = os.path.join(root, self._dataset_dir)
        print("dataset path: ", self._dataset_path)
        self._domain = "cat_individual_images"
