...
class BirdIndividualID(DatasetBase):
    """
    Bird Individual ID Dataset
    
    Reference:
    [Add your reference paper here if available]
    
    Dataset statistics:
        - images: 747 (train) + 193 (query) + 335 (gallery)
        - identities: 15 (train) + 11 (query) + 11 (gallery)
    """
    def __init__(self, cfg, domain_label, verbose=True):
        self._dataset_dir = "BirdIndividualID"
        root = cfg.DATASET.ROOT
        self._dataset_path = os.path.join(root, self._dataset_dir)
        print("dataset path: ", self._dataset_path)
        self._domain = "bird_individual_id"
