...
class AmvrakikosTurtles(DatasetBase):
    """
    Amvrakikos Turtles Dataset
    
    Reference:
    [Add reference paper here if available]
    
    Dataset statistics:
        - images: 120 (train) + 20 (query) + 60 (gallery)
        - identities: 30 (train) + 20 (query) + 20 (gallery)
    """
    def __init__(self, cfg, domain_label, verbose=True):
        self._dataset_dir = "AmvrakikosTurtles"
        root = cfg.DATASET.ROOT
        self._dataset_path = os.path.join(root, self._dataset_dir)
        print("dataset path: ", self._dataset_path)
        self._domain = "amvrakikos_turtles"
    ...
    def read_data(self, data_dir, relabel=False):
        ...
        for img_p in data_paths:
            ...
            try:
                ...
                # Camera ID is the second component (should be "-1" based on our renaming)
