...
class BelugaID(DatasetBase):
    """
    BelugaID Dataset
    
    Reference:
    Chelak, I., Körschens, M., Schneider, S., Denzler, J., & Freytag, A. (2020). 
    BelugaID: Beluga Whale Re-identification.
    
    URL:
        https://beluga.isosec.ca/
    
    Dataset statistics:
        - images: 1593 (train) + 344 (query) + 720 (gallery)
        - identities: 561 (train) + 227 (query) + 227 (gallery)
    """
    def __init__(self, cfg, domain_label, verbose=True):
        self._dataset_dir = "BelugaID"
